...
def read(sock, 需要查看的文件):
    ...
    try:
        while True:
            ...
            # 重启
            if file_n == 0 and boot_run.无文件时_更新是否重启:
                machine.reset()
...
def 子线程():
    ...
    wifi.conn_thr(boot_run.ssid, boot_run.pwd)
    ...
    while True:
        try:
            ip, 更新端口, 日志端口 = tl.get_ip_更新服务器(boot_run.广播端口)
            break
        except:
            time.sleep_ms(500)
    更新端口 = int(更新端口)
    ...
    rgb.write()
    t0 = time.ticks_ms()
    file_md5 = tl.get_files_md5("/", boot_run.忽略的文件和目录)
